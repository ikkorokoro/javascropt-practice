...
/* コンテキスト内で宣言した変数や関数の定義をコード実行前にメモリーに配置すること
  ホイスティングはコンテキストが生成されるごと宣言部分がメモリスペースに確保される
*/

// 関数
/*  以下のようにa();の呼び出しを関数の上に持ってきても関数を実行することができる
  これは関数を一度定義するとメモリ上に記憶されるのでa();を関数の上に記述しても実行することができる
*/
a();
function a() {
...
let constの場合、ReferenceErrorが起きる
...
function d() {
  console.log(e);
  var e = 0;//出力はundefineとなり, let、constの場合エラーが起きる
...
//関数式
/* この定義方法では変数の書き方と同じなため、
変数(let, const)のホイスティングの挙動と同じになるため以下コードの結果はReferenceErrorとなる */
g();
const g = function() {
